...
### Preserve the Nix installation made by `nix-installer`

On `release-26.05`, `nix.enable` defaults to `false`. Leave it false and do not set `nix.settings`: enabling the module makes System Manager own `/etc/nix/nix.conf` with `replaceExisting = true`, which would replace the file installed by `nix-installer`. This is the exact opt-in behavior in the release branch, not merely an unreleased-main-branch proposal. Source: [release-26.05 Nix compatibility module](https://github.com/numtide/system-manager/blob/05e08c6dd739d7f3204e71322594bb8095334cfb/nix/modules/upstream/nixpkgs/nix.nix#L7-L38).
